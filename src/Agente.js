...
const mensajesWhatsApp = {
    amazonas: "¡Hola! Estaba hablando con La Rana y siento el llamado del Amazonas. Me gustaría diseñar mi viaje para encontrar raíz y equilibrio.",
    macizo: "¡Hola! Estaba hablando con La Rana y siento el llamado del Macizo Colombiano. Me gustaría diseñar mi viaje para mi proceso de renacer e intención.",
    guainia: "¡Hola! Estaba hablando con La Rana y siento el llamado de Guainía. Me gustaría diseñar mi viaje para conectar con el amor y las relaciones.",
    sierranevada: "¡Hola! Estaba hablando con La Rana y siento el llamado de la Sierra Nevada. Me gustaría diseñar mi viaje para activar mi manifestación.",
...
export default function Agente({ nombre }) {
    ...
    const handleTerritorioClick = (terrKey, context) => {
        ...
        if (context === 'conocer') {
            setTimeout(() => {
                const keyToUse = terrKey === 'sierranevada' ? 'sierra' : (terrKey === 'bogota' ? 'sabanadebogota' : (terrKey === 'medellin' ? 'antioquia' : terrKey));
                let desc = t(`agente.${keyToUse}`);
                if (desc === `agente.${keyToUse}`) desc = t(`agente.${terrKey}`);
...
Tu conocimiento de los 8 pilares conceptuales:
- Macizo Colombiano / San Agustín: Renacer · Intención (Sembrar una intención, soltar ciclos concluidos y abrir espacio para lo nuevo).
                            ...
                            <div key={i} style={{ textAlign: m.rol === 'usuario' ? 'right' : 'left', marginBottom: '12px' }}>
                                <div style={{
                                    ...
                                    {m.tipo === 'menu' ? (
                                        ...
                                        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginTop: '5px' }}>
                                            {['amazonas', 'macizo', 'guainia', 'sierranevada', 'pacifico', 'putumayo', 'sabanadebogota', 'antioquia'].map(terr => (
