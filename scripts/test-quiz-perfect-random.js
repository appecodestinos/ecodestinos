...
console.log('=== MATRIZ PERFECTA (1-factorización K8) ===');
MAPEO_PREGUNTAS_PERFECTO.forEach((pregunta, i) => {
  console.log(`Q${i}: ${JSON.stringify(pregunta)}`);
});

function calcularPuntajesAleatorios(respuestas, matriz) {
  const puntajes = {};
...
function main() {
  ...
  console.log(`\nTotal combinaciones: ${combinaciones.length}`);
  console.log(`Lógica: +1 a 1 destino aleatorio de la pareja\n`);
  ...
  combinaciones.forEach(respuestas => {
    for (let sim = 0; sim < SIMULACIONES_POR_COMBINACION; sim++) {
      const [top1, top2] = calcularPuntajesAleatorios(respuestas, MAPEO_PREGUNTAS_PERFECTO);
      top1Count[top1]++;
      top2Count[top2]++;
      top1or2Count[top1]++;
      top1or2Count[top2]++;
